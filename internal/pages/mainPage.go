...
import (
	...
	"github.com/ktails/ktails/internal/tui/logstream"
...
type MainPage struct {
	...
	// each, merged into a single scrollback; logSup owns the live
	// stream/scanner/generation per source, keyed the same way as
	// podLogs' sources.
	podLogs     *models.LogPage
	showLogs    bool
	logsFocused bool
	logSup      *logstream.Supervisor
...
func (m *MainPage) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	// Log-stream messages are the logstream.Supervisor's to interpret;
	// MainPage only applies the resulting line/stream-ended update to the
	// render model.
	if upd, cmd, handled := m.logSup.Handle(msg); handled {
		m.applyLogUpdate(upd)
		return m, cmd
	}
...
// applyLogUpdate applies a logstream.Supervisor update to the Log pane's
// render model: a new line for one source, or that source's stream ending.
func (m *MainPage) applyLogUpdate(upd *logstream.Update) {
	if upd == nil {
		return
	}
	if upd.Closed {
		m.podLogs.SetStreamEnded(upd.SourceKey, upd.Err)
		return
	}
	m.podLogs.AppendLine(upd.SourceKey, upd.Line)
}
...
func (m *MainPage) openLogsForRows(rows []msgs.Row) tea.Cmd {
	...
	opened, closed, openCmds := m.logSup.Reconcile(targets)
	for _, key := range closed {
		m.podLogs.RemoveSource(key)
	}
	for _, t := range opened {
		m.podLogs.AddSource(t.Key, t.Pod, t.Namespace, t.Context, t.Container)
	}
