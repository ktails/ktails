...
func row(context, namespace string) msgs.Row {
	return msgs.Row{Context: context, Namespace: namespace}
}
...
func TestFilterRowsByNamespace_ChecksSelectedOnly(t *testing.T) {
	rows := []msgs.Row{row("ctx1", "default"), row("ctx1", "kube-system")}
...
func TestFilterRowsByNamespace_MultiContextIndependence(t *testing.T) {
	rows := []msgs.Row{row("ctx1", "default"), row("ctx1", "kube-system"), row("ctx2", "default"), row("ctx2", "kube-system")}
	...
	for _, r := range got {
		if r.Context == "ctx1" && r.Namespace != "default" {
